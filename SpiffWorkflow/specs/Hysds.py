...
class Hysds(TaskSpec):
    ...
    task = Hysds(spec, 'Ping', args=["ping", "-t", "1", "127.0.0.1"])
    ...
    def _try_fire(self, my_task, force = False):
        ...
        jobs = [self.args[0]]
        group_res = group(submit_job.s(job).set(queue="jobs_processed") for job in jobs)()
        while True:
        ...
        # harvest results in a group then return
        res = GroupResult(id=uuid.uuid4(), results=[AsyncResult(id[0]) for id in task_ids])
        while True:
            ready = res.ready()
            if ready: break
            time.sleep(5)
        my_task.results = [i for i in res.join(timeout=10.)]
        return True
    ...
    def _update_state_hook(self, my_task):
        if not self._try_fire(my_task):
            ...
            return
        super(Hysds, self)._update_state_hook(my_task)
